      ...
      <div className="card w-full max-w-lg mx-auto text-left shadow-xl rounded-2xl p-6 bg-white/90">
        <div className="mb-6">
          ...
          <div className="text-blue-700 text-base mb-1">
            Capture a win, a wobble, or a moment that matters.{' '}
            <span className="font-semibold">Special Check-Ins</span> help you tune your day to your
            real life. <span className="ml-1">✨</span>
          </div>
          <div className="text-gray-500 text-sm">
            No schedule required—just a label and a mood. DayTune celebrates your rhythm, not just
            your plans.
          </div>
        </div>
        <div className="flex gap-3 mb-6">
          <button
            className="px-4 py-2 bg-gray-200 text-blue-700 rounded-full shadow hover:bg-gray-300 transition"
            onClick={onBack}
          >
            Back to dashboard
          </button>
          <button
            className="px-4 py-2 bg-blue-600 text-white rounded-full shadow hover:bg-blue-700 flex items-center gap-2 transition font-semibold"
        ...
        {showAdd && (
          <div className="mb-8 p-6 bg-blue-50 border border-blue-200 rounded-xl shadow-lg animate-fadeIn">
            <div className="text-lg font-semibold mb-2 flex items-center gap-2">
              New Special Check-In <span className="text-xl">🔔</span>
            </div>
            <div className="text-gray-600 text-sm mb-3">
              What&apos;s this moment about? (e.g. After workout, Before bed, Big win, Needed a break…)
            </div>
            <input
              type="text"
              value={form.label}
        ...
        <div>
          {loading ? (
            <div>Loading...</div>
          ) : specialCheckins.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 animate-fadeIn">
              <span className="text-5xl mb-2">🌱</span>
              <div className="text-blue-700 text-lg font-semibold mb-1">
                No special check-ins yet.
              </div>
              <div className="text-gray-500 text-sm mb-2">
                Start by adding a moment that matters to you. DayTune is here to celebrate your
                real-life rhythm!
              </div>
            </div>
          ) : (
            <ul className="space-y-4">
              {specialCheckins.map((c) => {
                ...
                return (
                  <li
                    key={c.id}
                    className="flex items-center justify-between border rounded-xl px-4 py-3 bg-white shadow-md transition-all duration-300 animate-fadeIn"
                  >
                    <div>
